const DB_VERSION = 17; // Define the database version here
...
async function getRecordById(dbName, storeName, id) {
  console.log("Fetching record with ID:", id, "Type:", typeof id);
  const db = await openIndexedDB(dbName, DB_VERSION); // Adjust the version number as needed
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([storeName], "readonly");
    const store = transaction.objectStore(storeName);
...
async function deleteRecordFromIndexedDB(dbName, storeName, id) {
  const db = await openIndexedDB(dbName, DB_VERSION);
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([storeName], "readwrite");
    const store = transaction.objectStore(storeName);
